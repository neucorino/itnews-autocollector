...
**Version 1.3.2** — 複数 RSS ソース対応とユーザー関心トピックに基づくパーソナライズスコアリングを追加。日付パース・Gemini リトライ・記事 ID 重複防止により、マルチソース運用の安定性を強化しました。
...
- **コア機能**: RSS取得、Google Geminiによる記事の要約・スコアリング、重要度ベースのメール通知
- **API提供**: FastAPIによるREST API実装で、外部アプリからのデータ参照が可能
- **運用設計**: Dockerによる環境分離と、ヘルスチェック監視による自動復旧
- **高度な拡張**: 関心トピック連動のパーソナライズランキングと、堅牢な分析パイプライン
...
| 改善内容 | 詳細 |
|---------|------|
| **複数 RSS ソース対応** | TechCrunch / The Verge / Ars Technica / GitHub Blog を追加し、Hacker News を含む計 5 ソースから収集 |
| **パーソナライズスコアリング** | `USER_PREFERENCES` により関心トピックをプロンプトへ反映し、重要度判定をユーザー志向に調整 |
| **日付パースの堅牢化** | `parse_rss_date` で RFC 822 / ISO 8601 に対応し、失敗時は現在時刻へフォールバック |
| **バッチ処理フローの見直し** | ソース別上限（`SOURCE_FETCH_LIMIT`）と全ソース集約後の一括分析・ランキングへ変更 |
| **Gemini リトライ強化** | 503 対応・指数バックオフ・失敗時ダミーレコードにより、一時障害でもバッチ全体を継続 |
| **記事 ID 重複の防止** | `INSERT OR IGNORE` 時の誤 ID 割当を修正し、`(article_id, batch_id)` UNIQUE 制約で二重分析を防止 |
...
- **一括処理パイプラインの最適化 (v1.3.2)**: 計5つの複数RSSソースへの拡張に伴い、ソース別上限（SOURCE_FETCH_LIMIT）で制御。全記事を集約してからLLM分析・ランキングを行う効率的なフローへ改善。

### 2. マルチソース運用を想定した「堅牢性とデータ保護」の徹底

- **防御的データ設計と重複排除 (v1.3.2)**: DBのUNIQUE制約・ユニークインデックスの追加、分析前のID重複排除、INSERT OR IGNORE時のID再取得処理の適正化により、マルチソース化によるデータの不整合や二重分析を完全に防止。
- **高度なエラーハンドリング (v1.3.2)**: 起動時の設定検証（フェイルファスト）や、コンテキストマネージャーによる確実なリソース解放を徹底。さらにGemini APIの503エラー等に対する指数バックオフリトライを強化し、失敗時はダミーレコードを生成してバッチを止めずに継続する運用の安定性を実現。
...
- **関心トピック連動スコアリング (v1.3.2)**: USER_PREFERENCES をプロンプトへ動的注入。Gemini 2.5 Flash-Lite を活用し、低コストかつユーザー個人の関心に最適化された重要度評価に対応。
- **段階的鮮度減衰モデル**: ニュースの時間的価値を考慮し、経過時間に応じてスコアを段階的に低下。公開日時・IDを用いた安定ソートと組み合わせ、一貫性のある上位ランキングを算出。
- **日付パースの正規化 (v1.3.2)**: 複数ソースで混在するRFC 822 / ISO 8601両形式の公開日時を parse_rss_date で安全に正規化し、パース失敗時は現在時刻へ自動フォールバック。
